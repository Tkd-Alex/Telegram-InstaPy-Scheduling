...
def persistend_job(bot, job_queue, chat_data, days, chat_id, message_id=None, db_persist=True):
    if chat_data['tmpjob']['job_name'] not in chat_data:
        ...
        if chat_data['tmpjob']['days'] == "Everyday":
            job = job_queue.run_daily(exec_process, scheduled_time, context=context, name=job_name)
            ...
            job = job_queue.run_daily(exec_process, scheduled_time, days=tuple(chat_data['tmpjob']['days']), context=context, name=job_name)
    ...
    if str(update.message.chat_id) in dict_settings['allowed_id']:
        try:
            ...
            if job_name in chat_data and job_name in process_array:
                ...
                update.message.reply_text("Sorry, job named <b>{}</b> was not found.".format(job_name), parse_mode='HTML')
        except (IndexError, ValueError):
...
def main(setting_file='settings.json'):
    ...
    dp.add_handler(CommandHandler("reload", reload_jobs, pass_job_queue=True, pass_chat_data=True))
